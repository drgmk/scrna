...
class CellTypeMarkers:
        ...
        for cell_type, cell_data in self.data.items():
            ...
            if 'genes' in cell_data:
                if self.case.lower() == 'upper':
                    ...
                    cell_data['genes'] = [gene.capitalize() for gene in cell_data['genes']]
                else:
                    print(f"Warning: Unknown case option '{self.case}'. Valid options are 'upper', 'title', or None.")
                    
            # Convert secondary genes
            if 'genes_secondary' in cell_data:
                if self.case.lower() == 'upper':
                    cell_data['genes_secondary'] = [gene.upper() for gene in cell_data['genes_secondary']]
                elif self.case.lower() == 'title':
                    cell_data['genes_secondary'] = [gene.capitalize() for gene in cell_data['genes_secondary']]
    ...
    def filter_genes(self, gene_names: List[str], cell_types: Optional[List[str]] = None, verbose: bool = True) -> None:
        ...
        for cell_type in cell_types:
            ...
            filtered_secondary = [gene for gene in original_secondary if gene in gene_set]
            missing_secondary = [gene for gene in original_secondary if gene not in gene_set]
    ...
    def to_dict(self, include_secondary: bool = False) -> Dict[str, List[str]]:
        ...
        for cell_type in self.data.keys():
            result[cell_type] = self.get_markers(cell_type, include_secondary=include_secondary)
        return result
    ...
    def to_pandas(self, include_secondary: bool = False):
        ...
        try:
            ...
            raise ImportError("pandas is required for to_pandas method. Install with: pip install pandas")
