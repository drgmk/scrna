...
from hgtc_toolkit.src.datascience.orchestration.BaseClasses import BaseMethod, BaseOperation
    ...
    class metrics(BaseMethod):
        ...
        def parameterDerivatives(CONFIG, UID, **kwargs):
            ...
            for file in file_keys:
                if file in pars:
                    pars.update({file: os.path.join(mpars['user'],pars[file]),})
...
class doublets(BaseOperation):
    class scrublet(BaseMethod):
        ...
        def main(adata: Any, pars: Dict[str, Any], **kwargs) -> Any: 
            ...
            sc.pl.scrublet_score_distribution(adata, frameon = False, save=f"doublets.png")
...
class filtering(BaseOperation):
    ...
    def preamble(cls, adata, pars, **kwargs):
        ...
        qcargs = {'adata':adata, 'qc_vars':pars['features'], 
                    'percent_top':[1], 'log1p':True, 'inplace':True}
        ...
        if 'doublet' in adata.obs.keys():
            ...
            vargs={'adata':adata, 'x':'total_counts', 'y':'n_genes_by_counts', 
                    'color':['doublet'], 'save':save_to}
    ...
    def generate_feature_mask(adata, pars):
        ...
        if 'feature' in pars: features.append(pars['feature'])
        
        for feature in features:
            if not feature or feature not in pars.get("gene_id", {}): continue
    ...
    class pct_counts_linear(BaseMethod):
        ...
        parameters = {
            ...
            'target_feature_id':{
                'type':'dict',
                'display':False,
                'default':{'mitochondrial':'pct_counts_mitochondrial', 
        ...
        def parameterDerivatives(cls, CONFIG, UID, **kwargs) -> Any:
            ...
            if not species in ["human", "mouse"]:
                print("InputError: <experimentData.species> must be 'human' or 'mouse'.")
                return None
            else:
        ...
        def main(cls, adata: Any, pars: Dict[str, Any], **kwargs) -> Any:
            ...
            fig = sns.relplot(x='pct_counts_in_top_1_genes', y=ftID, col='sampleID',
                            data=adata.obs, size=0.1, col_wrap=4)
            ...
            mask = scfunc.trim_outliers(
                adata, groupby='sampleID', pct=pars['confidence_interval'],
...
class annotation(BaseOperation):
    class cellTypist(BaseMethod):
        ...
        def main(cls, adata: Any, pars: Dict[str, Any], **kwargs) -> Any:
            ...
            markers = markers.rename(columns={"cell_type": "source", "genesymbol": "target", f"{pars['species']}_sensitivity": "weight"})
